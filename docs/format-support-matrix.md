...
|---|---|---|---|---|---|---|
| 1 | 1080p60 | 1080p60 | ❌ | — | none | **Hardware-limited on Zybo Z7-20 (-1 speed grade).** Attempted 2026-05-31: Vivado DRC AVAL-46 — rgb2dvi internal MMCM FVCO = 148.5 × 10 = 1485 MHz, exceeds -1 speed grade 1200 MHz max. Workarounds: (a) replace rgb2dvi with HDMI TX IP supporting 5× signaling; (b) migrate to -2 speed grade silicon (TE0720 production target is -2, so this row is plausible on the production hardware); (c) accept 1080p30 / 1080p50 (different rows) on Zybo. Phase A historic ✅ claim used a different output topology not currently in build. |
