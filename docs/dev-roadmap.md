...
| **D** | Frame rate conversion (FRC). Cadence convert per banked rate matrix (3:2 pulldown, 5:2, 6:5, 4:5, 2:1, 1:2, slip). 3:2 cadence auto-detect for inverse-telecine. | 2–4 weeks | 🟡 **D iter 1 done (2026-05-14)** — output-pixel-clock genlock fix: `clk_wiz_pixclk_out/clk_in1` re-sourced from `dvi2rgb_0/PixelClk` (recovered 148.5 MHz) instead of `zynq_ps/FCLK_CLK2`. Same-source input + output MMCMs → no crystal drift → temporal tearing seam eliminated. Bench-confirmed via MS2109 HDMI capture (`build/ila-capture/phase-d/genlock-fix-capture.jpg`) — color bars dramatically cleaner vs C.1; residual noise concentrated in PLUGE/dark regions (likely scaler-kernel territory for C.3). **Known regression to address next:** VTC's CTL register clears when `clk_wiz_pixclk_out` loses lock during source disconnect; firmware sets CTL.gen_enable once at boot and doesn't re-init, so picture stays dark after replug until `vdma_init.elf` reloads. Fix: poll dvi2rgb `pLocked` from PS and re-init VTC on the low→high transition. Proper FRC (3:2, 5:2, etc.) still pending. |
