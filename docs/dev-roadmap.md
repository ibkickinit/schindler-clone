...
| **D** | Frame rate conversion (FRC). Cadence convert per banked rate matrix (3:2 pulldown, 5:2, 6:5, 4:5, 2:1, 1:2, slip). 3:2 cadence auto-detect for inverse-telecine. | 2–4 weeks | 🟡 **D iter 1 done (2026-05-14)** — output-pixel-clock genlock fix: `clk_wiz_pixclk_out/clk_in1` re-sourced from `dvi2rgb_0/PixelClk` instead of `zynq_ps/FCLK_CLK2`. Same-source input + output MMCMs → no crystal drift → temporal tearing seam eliminated. Bench-confirmed (`build/ila-capture/phase-d/genlock-fix-capture.jpg`).  🟡 **D iter 2 done (2026-05-14)** — firmware tracks S2MM PARK_PTR and logs source-quiet/restored events to UART (informational). **Auto-recovery attempted and abandoned:** tried VTC re-init + `XAxiVdma_DmaStop`/`DmaStart` every quiet tick — S2MM stays wedged across a disconnect, the simple driver helpers don't unstick it. Until a proper fix lands, manual recovery is `xsct tcl/program_phase_b_full.tcl`. **Two open follow-ups:** (1) **Auto-recovery** — needs a BD-side signal of source-alive (route `dvi2rgb_0/pLocked` to an AXI GPIO, *or* gate input-pipeline `aresetn` on pLocked at the hardware level), plus a deeper VDMA reset than the driver provides. (2) **VTC↔source vsync phase alignment** — Phase D fixes the *rate* mismatch but the two clock domains still have arbitrary vertical phase at boot; result is occasional rolled frames (stable but vsync offset). Make VTC's vsync trigger from the source's vsync rather than free-running. Proper FRC (3:2, 5:2, etc.) still pending. |
