...
- **Phase 3 (phase-e1-pll-spike):** ✅ **RESOLVED 2026-06-01 by ARCHIVE, not merge.** A merge attempt was started (scaler_h/v + XDC region 1 resolved) then aborted after confirming the key fact: **all v1-relevant work is already on iter5 trunk.** phase-e1's only unique content is the Phase-E2 MMCM tracking (v2, scope-fenced), so merging iter5 in would be throwaway — the "current base" decays as iter5 moves, and the real resync is better done fresh from then-current iter5 at E2-revival time. Branch deleted (local+remote); preserved as tag `archive/phase-e1-pll-spike` @ `d7d2acf`. Closes task #65. Revive E2 with `git checkout -b <new> archive/phase-e1-pll-spike` → merge-from-iter5.
