...
- SDI reference in (1 BNC) — Semtech GS3470 receiver (confirmed over prior GS2971-class candidate). Locks PLL to recovered SDI clock, extracts VITC for timecode, format auto-detect drives frame-rate selection. GS3470 is newer, lower power, smaller package. **Note:** the 2×2 input mux feature (originally pitched for passive loop-through) is unused in V1 — passive loop-through was dropped from the broadcast tier in the 2026-05-11 connector simplification.
