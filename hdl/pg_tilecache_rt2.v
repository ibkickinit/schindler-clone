...
module pg_tilecache_rt2 #(
    ...
    reg [TIDW-1:0] tag[0:NTILE-1]; reg vld[0:NTILE-1]; reg [SLW-1:0] rr;
    ...
    // combinational "is tile t resident? -> slot"
    function automatic [SLW:0] look; input [TIDW-1:0] t; input integer dummy;
        integer i; reg h; reg [SLW-1:0] s; begin h=0;s=0;
        for(i=0;i<NTILE;i=i+1) if(vld[i]&&tag[i]==t) begin h=1;s=i[SLW-1:0]; end look={h,s}; end
    endfunction
    // NOTE: look() reads vld/tag; used only inside always@* blocks (sensitive) — never in a wire-assign.
    ...
    reg gh00,gh10,gh01,gh11; reg [SLW-1:0] gs00,gs10,gs01,gs11; integer gi;
    always @* begin
        gh00=0;gs00=0;gh10=0;gs10=0;gh01=0;gs01=0;gh11=0;gs11=0;
        for(gi=0;gi<NTILE;gi=gi+1) begin
            if(vld[gi]&&tag[gi]==tidf(cpx0,cpy0)) begin gh00=1;gs00=gi[SLW-1:0]; end
            if(vld[gi]&&tag[gi]==tidf(cpx1,cpy0)) begin gh10=1;gs10=gi[SLW-1:0]; end
            if(vld[gi]&&tag[gi]==tidf(cpx0,cpy1)) begin gh01=1;gs01=gi[SLW-1:0]; end
            if(vld[gi]&&tag[gi]==tidf(cpx1,cpy1)) begin gh11=1;gs11=gi[SLW-1:0]; end
        end
    end
    wire c_all = gh00&gh10&gh01&gh11;
    wire [23:0] cr00=b00[baddr(gs00,cpx0,cpy0)], cr10=b10[baddr(gs10,cpx1,cpy0)],
                cr01=b01[baddr(gs01,cpx0,cpy1)], cr11=b11[baddr(gs11,cpx1,cpy1)];
    wire [23:0] cg00=cx_[0]?(cy_[0]?cr11:cr10):(cy_[0]?cr01:cr00);
    wire [23:0] cg10=cx_[0]?(cy_[0]?cr01:cr00):(cy_[0]?cr11:cr10);
    wire [23:0] cg01=cx_[0]?(cy_[0]?cr10:cr11):(cy_[0]?cr00:cr01);
    wire [23:0] cg11=cx_[0]?(cy_[0]?cr00:cr01):(cy_[0]?cr10:cr11);
    wire [23:0] cpp00=cg00, cpp10=ce_x?cg00:cg10, cpp01=ce_y?cg00:cg01,
                cpp11=ce_x?cpp01:(ce_y?cpp10:cg11);
    // 1-deep pipeline: when the staged coord's tiles are resident (or matte), EMIT it AND accept the
    // next coord in the SAME cycle -> 1 output/clock. Stall (hold) only on a genuine miss.
    wire stage_ready = cin_ ? c_all : 1'b1;                     // staged coord can emit?
    assign c_ready = (!out_valid || out_ready) && (!c_busy || stage_ready);

    always @(posedge clk) begin
        if(!rstn) begin c_busy<=0; out_valid<=0; end
        else begin
            if(out_valid && out_ready) out_valid<=0;
            if(!out_valid || out_ready) begin
                if(!c_busy) begin
                    if(c_valid) begin cx_<=c_x;cy_<=c_y;cfx_<=c_fx;cfy_<=c_fy;cin_<=c_inwin;csb_<=c_sb; c_busy<=1; end
                end else if(stage_ready) begin                  // emit staged + accept next (1/clk)
                    out_valid<=1; out_inwin<=cin_; out_fx<=cfx_;out_fy<=cfy_;out_sb<=csb_;
    ...
    reg pr00,pr10,pr01,pr11; integer pj;
    always @* begin pr00=0;pr10=0;pr01=0;pr11=0;
        for(pj=0;pj<NTILE;pj=pj+1) begin
            if(vld[pj]&&tag[pj]==pt00) pr00=1; if(vld[pj]&&tag[pj]==pt10) pr10=1;
            if(vld[pj]&&tag[pj]==pt01) pr01=1; if(vld[pj]&&tag[pj]==pt11) pr11=1;
        end
    end
    ...
    reg [TIDW-1:0] ua_tid; reg [11:0] ua_tx, ua_ty;        // first unavailable tile (priority)
    always @* begin
        if(!av00) begin ua_tid=pt00; ua_tx=ppx0>>LTILE; ua_ty=ppy0>>LTILE; end
        else if(!av10) begin ua_tid=pt10; ua_tx=ppx1>>LTILE; ua_ty=ppy0>>LTILE; end
        else if(!av01) begin ua_tid=pt01; ua_tx=ppx0>>LTILE; ua_ty=ppy1>>LTILE; end
        else begin ua_tid=pt11; ua_tx=ppx1>>LTILE; ua_ty=ppy1>>LTILE; end
    end
    wire cur_done = !pin_ || all_av;
    assign pf_ready = !p_busy || cur_done;

    reg [2*HT-1:0] fcw; reg [SLW-1:0] fill_slot;
    wire [BAW-1:0] fwa = (fill_slot<<(2*HT)) | fcw;

    always @(posedge clk) begin
        if(!rstn) begin p_busy<=0; fetch_req<=0; rr<=0; pend_v<=0;
            for(pj=0;pj<NTILE;pj=pj+1) vld[pj]<=0; end
        else begin
            fetch_req<=0;
            // issue one fill for the first unavailable tile of the current coord (DMA free)
            if(p_busy && pin_ && !all_av && !pend_v) begin
                fetch_req<=1; fetch_tx<=ua_tx; fetch_ty<=ua_ty;
                fill_slot<=rr; fcw<=0; pend_tid<=ua_tid; pend_v<=1;
            end
            ...
            if(pend_v && fill_valid) begin
                ...
                fcw<=fcw+1'b1;
                if(fill_last) begin tag[rr]<=pend_tid; vld[rr]<=1'b1; rr<=rr+1'b1; pend_v<=0; end
            end
        end
    end
endmodule
