...
module pg_raster_to_tile_tb;
    ...
    reg [23:0] s_td; reg s_tv=0, s_axis_tuser=0, s_axis_tlast=0; wire s_tr;
    ...
    initial begin
        ...
        for(fr=0;fr<TILE;fr=fr+1) for(fc=0;fc<IN_W;fc=fc+1) begin
            @(posedge clk); s_td<=(fr*100+fc); s_tv<=1; s_axis_tuser<=(fr==0&&fc==0); s_axis_tlast<=(fc==IN_W-1);
            while(!s_tr) @(posedge clk);
        end
        @(posedge clk); s_tv<=0; s_axis_tuser<=0; s_axis_tlast<=0;
