...
`endif
    ...
    // BURSTY DELIVERY (mimic the real AXI DataMover latency/backpressure the gap-free model hides):
    // gate PRODUCTION only; a pending beat is never lost (consumed-during-gap clears it -> clean gap).
    reg [5:0] gapc; always @(posedge clk) gapc <= gapc + 1'b1;
    wire dm_stall = (gapc[1:0] == 2'b00);        // stall 1 of 4 -> 75% delivery (deadlocks; gap-triggered)
    wire beat_go  = (!beat_valid || beat_ready) && !dm_stall;
    ...
    always @(posedge clk) begin
        if(!rstn) begin cq_cnt<=0; cq_wr<=0; cq_rd<=0; bidx<=0; loaded<=0;
            ...
            if(beat_valid && beat_ready && dm_stall) beat_valid<=1'b0;  // consumed during a gap -> clean gap
            if(beat_go) begin
                if(cont_row) begin
    ...
    // WEDGE DETECTOR: if u_dma backpressures the DataMover (beat_ready stuck 0) for a long time while
    // tiles are still outstanding, dump pg_tile_dma's internal state to reveal the exact deadlock.
    integer stuck=0, lastcn=0; reg dumped=0;
    always @(posedge clk) begin
        if(rstn && started) begin
            if(cn==lastcn) stuck<=stuck+1; else stuck<=0;
            lastcn<=cn;
            if(stuck==80000 && !dumped) begin     // consumer made no progress for a long time -> dump
                dumped<=1;
                $display("STALL @cn=%0d  DMA[ rx_act=%b rx_left=%0d nbits=%0d rx_pp=%b em_pp=%b f0=%b f1=%b emit_act=%b rx_pr=%0d epr=%0d ecol=%0d rcol=%0d rx_sub=%b rq_cnt=%0d ]  IO[ wreq=%b dm_req=%b beat_v=%b beat_r=%b fv=%b fl=%b cq_cnt=%0d o_valid=%b o_ready=%b ]",
                    cn, u_dma.rx_act, u_dma.rx_left, u_dma.nbits, u_dma.rx_pp, u_dma.em_pp, u_dma.full[0], u_dma.full[1],
                    u_dma.emit_act, u_dma.rx_pr, u_dma.epr, u_dma.ecol, u_dma.rcol, u_dma.rx_sub, u_dma.rq_cnt,
                    wreq, dm_req, beat_valid, beat_ready, fv, fl, cq_cnt, o_valid, o_ready);
                $finish;
            end
        end
    end
